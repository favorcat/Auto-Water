...
int R_sec = 5;            //모터 동작시간 기본 5초       (사용자가 가변저항으로 변경가능 5~30초)
int M_speed = 200;        //모터 동작속도 기본 255(최대) (사용자가 가변저항으로 변경가능 100~200)
...
void loop() {
  ...
  if (_time1 < millis() ) {    // 매 0.2 초마다 버튼 센서값 읽기 
    ...
    //--------------------------------------------------------
    for (int i = 0; i < 2; i++) {
      Sw[i] = digitalRead(pinSw[i]);
      if (Sw[i] == 0) { //모터 동작버튼이 눌러지면
        
        int ok = digitalRead(pinMoter[i][0]);
        Serial.print("버튼-");
        Serial.print(i+1);
        Serial.print(" 감지됨 ");
        if (Tank==0) Serial.print("물탱크 저수위경보 모터중지 ");
        Serial.print("모터동작상태=");
        Serial.println(ok);
        ...
        if ((ok!=1) && (Tank==1)) {  //물탱크에 물이 있고 모터가 동작중이 아니면   1:동작중 0:멈춤상태
          ...
          Serial.print(" ");
          Serial.print(R_sec);
          Serial.println("초 동작 시작 ... ");
          
          R_time[i] = R_sec;  //모터 동작시간 설정하기 (기본 5초)
          digitalWrite(pinMoter[i][0],HIGH);   // 모터동작 

          lcd_print( 1, 11+i);  // k=11  'Moter-1 : Start '
                                // k=12  'Moter-2 : Start '
        }
      }
    }   
  ...
  if (_time2 < millis() ) {    // 매 1 초마다 센서값 읽기 
    ...
    rr3 = map(r3, 0, 1023, 5,10 );    //모터 동작시간 5~10초
    rr4 = map(r4, 0, 1023, 100,200 ); //모터 스피드 100~200
   ...
   if (abs(cds-rr2)>2) { //CDS 조도값이  3이상 변할때 적용
    ...
    if (abs(R_sec-rr3)>1) { lcd_print( 1, 4); mc=4; } //모터 동작시간이 2이상 변할때 LCD에 이벤트 출력
